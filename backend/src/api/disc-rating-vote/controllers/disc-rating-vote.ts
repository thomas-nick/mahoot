...
const RATING_UID = 'api::disc-rating.disc-rating';
const VOTE_UID = 'api::disc-rating-vote.disc-rating-vote';
...
export default factories.createCoreController(VOTE_UID as any, ({ strapi }) => ({
  /**
   * POST /api/disc-rating-votes
   *
   * Idempotent per (rating, voter): if a vote already exists, update its
   * value. Always recomputes helpfulCount/notHelpfulCount on the parent
   * rating so callers don't have to.
   */
  ...
  /**
   * DELETE /api/disc-rating-votes/by-rating/:ratingDocumentId
   *
   * Removes the caller's vote on a given rating, if any.
   */
