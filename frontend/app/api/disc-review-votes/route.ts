...
/**
 * GET /api/disc-review-votes?ratings=docId1,docId2,…
 *
 * Returns the caller's vote on each rating: { votes: { [ratingDocumentId]: 1 | -1 } }
 */
export async function GET(request: Request) {
  ...
  const response = await fetch(`${STRAPI_URL}/api/disc-rating-votes?${params.toString()}`, {
    headers: { Authorization: `Bearer ${jwt}` },
    cache: "no-store",
  });
...
/**
 * POST /api/disc-review-votes  body: { ratingDocumentId: string, value: 1 | -1 }
 *
 * Idempotent per (voter, rating). Backend recomputes helpfulCount /
 * notHelpfulCount on the parent rating.
 */
export async function POST(request: Request) {
  ...
  const response = await fetch(`${STRAPI_URL}/api/disc-rating-votes`, {
    method: "POST",
...
/**
 * DELETE /api/disc-review-votes?rating=<documentId>
 */
export async function DELETE(request: Request) {
  ...
  const response = await fetch(
    `${STRAPI_URL}/api/disc-rating-votes/by-rating/${encodeURIComponent(ratingDocumentId)}`,
    {
      method: "DELETE",
      headers: { Authorization: `Bearer ${jwt}` },
      cache: "no-store",
